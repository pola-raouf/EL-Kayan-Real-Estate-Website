    ...
    public function store(Request $request)
{
    ...
    try {
        $user = auth()->user();
        $data['user_id'] = ($user->role === 'admin' && !empty($data['user_id'])) ? $data['user_id'] : $user->id;
...
public function cancelReservation(Property $property)
{
    ...
    try {
        ...
        if (!$reservation) {
            $logger->warning('Cancel reservation failed - no reservation found', [
                'property_id' => $property->id,
                'user_id' => $user->id,
            ]);
        ...
        if ($reservation->user_id !== $user->id && $user->role !== 'admin') {
            $logger->warning('Unauthorized reservation cancellation attempt', [
                'property_id' => $property->id,
